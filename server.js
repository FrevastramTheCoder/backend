...
// const rmdirAsync = promisify(fs.rm || fs.rmdir); // fs.rm since Node v14+
...
//   const requiredVars = ['JWT_SECRET', 'DB_USER', 'DB_PASS', 'DB_HOST', 'DB_NAME', 'DB_PORT'];
...
//     Variable: varName,
//     Status: 'PRESENT',
//     Value: varName.includes('PASS') ? '*****' : process.env[varName]
//   })));
// };
...
//     // Assuming your table has columns: id, properties (JSONB)
...
//   // Accept only .zip files (shapefile usually zipped)
...
//         // Create table if not exists with properties column JSONB and id SERIAL
//         // You can customize this to create table dynamically if needed
//         await client.query(`
...
//   try {
//     const dbRes = await pool.query('SELECT NOW(), pg_database_size(current_database()) AS db_size');
//     res.json({
//       status: 'healthy',
...
//         sizeMB: Math.round(dbRes.rows[0].db_size / 1024 / 1024)
//       }
//     });
//   } catch (err) {
//     res.status(503).json({
...
// const validateConfig = () => {
//   const requiredVars = [
//     'JWT_SECRET', 'DB_USER', 'DB_PASS', 'DB_HOST', 'DB_NAME', 'DB_PORT',
...
//     Variable: varName,
//     Status: 'PRESENT',
//     Value: varName.toLowerCase().includes('pass') ? '*****' : process.env[varName]
//   })));
// };
...
//   const is_verified = true; // social users considered verified

//   const insertRes = await pool.query(
//     `INSERT INTO users (name, email, is_verified, role, provider) 
//      VALUES ($1, $2, $3, $4, $5) RETURNING *`,
//     [name, email, is_verified, 'user', provider]
//   );
...
//     const user = userRes.rows[0];
//     if (!user.is_verified) return res.status(403).json({ error: 'Please verify your email before login' });
...
//     const resetToken = jwt.sign({ id: userRes.rows[0].id }, process.env.JWT_SECRET, { expiresIn: '1h' });

//     // Save reset token in DB (optional, to invalidate tokens if needed)
...
// // Reset password route (using token)
...
//     const updateRes = await pool.query(
//       'UPDATE users SET password = $1, reset_token = NULL WHERE id = $2 RETURNING id, email',
//       [hashedPassword, userId]
//     );
...
//   (req, res) => {
//     const token = jwt.sign({ id: req.user.id, email: req.user.email, role: req.user.role }, process.env.JWT_SECRET, { expiresIn: '24h' });
...
// // =============================================
// // SOCIAL AUTH ROUTES (Updated with session support)
// // =============================================
// app.get('/auth/facebook', passport.authenticate('facebook', { scope: ['email'] }));
