...
//   try {
//     const { dataset } = req.params;
//     const result = await pool.query(`SELECT id, ST_AsGeoJSON(geom) AS geom FROM "${dataset}" ORDER BY id ASC`);
//     const features = result.rows.map(row => {
//       try {
...
//           type: 'Feature',
//           id: row.id,
//           properties: {}, // Add properties if needed from other columns
//           geometry
//         };
//       } catch (err) {
...
//       return res.status(404).json({ error: 'No valid features found' });
//     }
...
//     const columns = keys.map(k => `"${k}"`).join(', ');
//     const result = await pool.query(
//       `INSERT INTO "${dataset}" (${columns}, geom) VALUES (${placeholders}, ST_GeomFromGeoJSON($${keys.length + 1})) RETURNING *`,
//       [...values, JSON.stringify({ type: 'Point', coordinates: [0, 0] })] // Default geometry, replace with actual if provided
//     );
//     const { id, geom, ...recordProperties } = result.rows[0];
//     res.json({
//       message: 'Item uploaded!',
...
//           type: 'Feature',
//           id: row.id,
//           properties: {}, // Expand with other columns if needed
//           geometry
//         };
//       } catch (err) {
//         console.error(`JSON parse error for id ${row.id}:`, err.stack);
...
//   windowMs: 15 * 60 * 1000,
//   max: 100,
//   keyGenerator: (req) => rateLimit.ipKeyGenerator(req) // Use helper for IPv6 support
// });
// app.use(limiter);
...
//     res.status(500).json({ error: 'Internal server configuration error. Please contact support.', stack: process.env.NODE_ENV === 'development' ? err.stack : undefined });
//   } else {
//     res.status(err.status || 500).json({ error: err.message || 'Internal Server Error', stack: process.env.NODE_ENV === 'development' ? err.stack : undefined });
//   }
// });
...
const cors = require('cors');
const session = require('express-session');
const RedisStore = require('connect-redis')(session); // Initialize connect-redis with express-session
const { createClient } = require('redis');
const jwt = require('jsonwebtoken');
const bcrypt = require('bcrypt');
const passport = require('passport');
...
// Redis Client Setup
let sessionStore = new session.MemoryStore(); // Default to MemoryStore
let redisErrorLogged = false;
const redisClient = createClient({
  url: process.env.REDIS_URL,
  socket: {
...
redisClient.on('ready', () => {
  console.log('✅ Redis Client Ready');
  sessionStore = new RedisStore({ client: redisClient }); // Initialize RedisStore after connection
  redisErrorLogged = false;
...
// Session Middleware
app.use(session({
  store: sessionStore, // Use sessionStore directly
  secret: process.env.SESSION_SECRET.trim(),
  resave: false,
  saveUninitialized: false,
  cookie: {
...
app.get('/auth/twitter/callback', passport.authenticate('twitter', {
  ...
  try {
    ...
    console.log(`DEBUG: Twitter auth callback successful, redirecting: ${req.user.email}`);
    res.redirect(`${process.env.CLIENT_URL}/social-login?token=${encodeURIComponent(token)}`);
  } catch (err) {
    console.error('DEBUG: Error in Twitter auth callback:', err.stack);
    res.redirect(`${process.env.CLIENT_URL}/social-login?error=auth_failed`);
  }
});
*/
